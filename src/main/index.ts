...
const defaultCreateParent = (): string => join(app.getPath('documents'), 'Thinking Canvas')
...
async function migrateLegacyUserData(): Promise<void> {
  ...
  for (const legacyName of ['bee claude', 'bee-claude']) {
    ...
    if (!(await exists(join(legacyDir, 'folders.json'))) && !(await exists(join(legacyDir, 'repos.json'))))
      continue
    try {
